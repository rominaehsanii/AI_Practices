...
def depthFirstSearch(problem):
    ...
    while not MyStack.isEmpty():
        ...
        for next in problem.getSuccessors(List[0]):
            ...
            # if next node isn't visited yet and it's goal point
            # returns path up to here
            if Name not in VisitedList:
                if problem.isGoalState(Name):
                    row = List[1] + [Path]
                    return row
                # otherwise, push this node to MyStack
                else:
                    MyStack.push((Name, List[1] + [Path]))
                VisitedList.append(Name)
